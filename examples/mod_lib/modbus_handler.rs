...
static MODBUS_UART_CELL: StaticCell<Mutex<RawMutex, Uart<'static, Blocking>>> = StaticCell::new();
...
/// Read a Modbus response within specified timeout
/// 
/// # Arguments
/// * `timeout_ms` - Maximum wait time in milliseconds
/// 
/// # Returns
/// * `Some(DataFrame)` if response received within timeout
/// * `None` if timeout occurred or error
pub async fn modbus_read_response(timeout_ms: u32) -> Option<DataFrame> {
    ...
    if let Some(uart_mutex) = get_modbus_uart() {
        ...
        loop {
            ...
            
            {
                ...
                // Try to read one byte at a time until no more data
                while bytes_read < buffer.len() {
                    let mut single_byte = [0u8; 1];
                    match uart_guard.read_buffered(&mut single_byte) {
                        Ok(_) => {
            ...
            // Check if we have minimum frame (address + function + CRC = 4 bytes)
            if bytes_read >= 4 {
                // Wait a bit for complete frame (Modbus RTU inter-frame delay)
                Timer::after(Duration::from_millis(5)).await;
                
                // Verify CRC
                if modbus_verify_crc(&buffer[..bytes_read]) {
                    // Parse frame
                    let mut frame = DataFrame::new();
                    frame.address = buffer[0];
                    frame.function = FunctionCode::Custom(buffer[1]);
                    frame.frame_length = bytes_read as u8;
                    
                    // Extract CRC (little-endian)
                    frame.crc = buffer[bytes_read - 2] as u16 | 
                               ((buffer[bytes_read - 1] as u16) << 8);
                    
                    // Extract data payload (exclude address, function, CRC)
                    let data_len = bytes_read - 4;
                    if data_len > 0 {
                        let _ = frame.data.extend_from_slice(&buffer[2..2 + data_len]);
                    }
                    
                    return Some(frame);
                }
                break; // Invalid CRC, exit
            }
...
fn modbus_frame_to_buffer(
    ...
    // 5. CRC (2 bytes) - Little Endian (low byte first, then high byte)
